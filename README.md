...
يُشغَّل النظام بنموذج **نسخة لكل عميل (single-tenant per instance)**: الكود واحد، ولكل عميل عملية تطبيق مستقلة وقاعدة بيانات مستقلة ومجلد ملفات مستقل ونطاق مستقل. البيئات الحالية:

| المستأجر | المنفذ المحلي |
|---|---|
| `radeef` (البيئة الأساسية) | 3000 |
| `rakan` | 3001 |
| `dar` | 3002 |
...
**حجم الكود:** نحو 90 صفحة، و86 ملف مسار API (أكثر من 80 مسارًا)، و54 موديل Prisma.
...
radeef/
├── prisma/
│   ├── schema.prisma            # مخطط قاعدة البيانات (54 موديل)
...
├── docs/                        # RUNBOOK.md, DATABASE.md, العروض والقوالب القديمة
...
- **الجلسة:** بعد تسجيل الدخول يُصدر السيرفر JWT موقّعًا (HS256) بـ `SESSION_SECRET` داخل cookie باسم `radeef_session` بخصائص `httpOnly` و`SameSite=Lax` و`Secure` في الإنتاج، وصلاحيتها 12 ساعة. لا يُحفظ الدور ولا معرف المستخدم في `localStorage`.
...
- **الملفات المرفوعة:** تُحفظ في `UPLOAD_DIR` خارج `public/`، وتُخدَّم فقط عبر المسار المحمي `/api/files/...` مع قائمة أنواع مسموحة وفحص توقيع الملف وحد 10MB (5MB للزوار). روابط `/uploads/...` القديمة تُعاد توجيهها داخليًا إلى المسار المحمي.
...
sudo ops/deploy.sh --mode docker --rollback dar # الرجوع للصورة السابقة (بلا تراجع عن قاعدة البيانات)
```
...
sudo -iu radeef /opt/radeef/src/ops/deploy.sh --rollback rakan
```
...
- **أسرار مسرّبة يجب تدويرها فورًا:** كانت كلمة مرور root للسيرفر وعنوانه وكلمة مرور Postgres وسرّ جلسة ثابت وكلمة مرور مدير افتراضية مكتوبة داخل سكربتات في هذا المستودع. أُزيلت من الملفات ونُقلت السكربتات إلى `ops/legacy/`، لكنها ما زالت في تاريخ git وأي نسخة سابقة. الخطوات في [`docs/RUNBOOK.md`](docs/RUNBOOK.md) (القسم 1).
...
- **الحضور:** تسجيل يومي، جداول عمل، طلبات تصحيح الحضور، ربط أجهزة البصمة عبر `biometricId`.
- **الإجازات والتأشيرات:** مسار موافقات، حساب الرصيد، تأشيرات خروج وعودة ونهائي.
- **الرواتب والمالية:** مسير شهري بالبدلات والمكافآت والخصومات وأقساط السلف، التسويات ونهاية الخدمة محسوبة على السيرفر، المطالبات وأوامر الدفع.
- **الإدارة القانونية:** السندات لأمر، العقود، القضايا، التحقيقات الإدارية.
