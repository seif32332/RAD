...
export default function NewClaimPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 401) return redirectToLogin();
      if (!res.ok) {
        const msg = await readApiError(res, 'تعذر حفظ المطالبة');
        setErrorMsg(msg);
        toast.error(msg);
        setIsLoading(false);
        return;
  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-8 md:py-12 mb-32 space-y-8">
        ...
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            ...
            إضافة مطالبة حادث جديدة
          </h1>
          <p className="text-slate-500 font-semibold mt-2 mr-16">اختر المركبة المتضررة وأدخل تفاصيل المطالبة والتأمين.</p>
        </div>
          ...
          <Section title="المركبة المتضررة" icon={<Truck size={16} className="text-indigo-600" />} badge="مطلوب">
            <div className="grid grid-cols-1 gap-6">
              <SearchableSelect name="vehicleId" value={formData.vehicleId} onChange={handleChange} label="اختيار السيارة (برقم اللوحة)" required accentColor="red"
                options={vehicles.filter(v => !v.isArchived).map(v => ({ label: `${v.plateNumber ?? ''} — ${v.brand ?? ''} ${v.modelYear ?? ''} (${v.vehicleCode ?? ''})${v.isArchived ? ' — مؤرشفة' : ''}`, value: v.id }))} />
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 p-4 md:py-5 md:px-12 flex justify-between items-center z-50">
        ...
        <button type="submit" form="claim-form" disabled={isLoading}
          className="px-10 py-3.5 text-[13px] font-black text-white bg-slate-900 rounded-[1.25rem] hover:bg-red-600 disabled:opacity-50 transition-all shadow-lg flex items-center gap-2">
          {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Save size={18} />}
          {isLoading ? 'جاري الحفظ...' : 'حفظ المطالبة'}
        </button>
      </div>
    </DashboardLayout>
  );
