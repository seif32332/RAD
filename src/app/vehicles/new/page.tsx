...
export default function NewVehiclePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await readApiError(res, 'تعذر حفظ المركبة');
        setErrorMsg(msg);
        toast.error(msg);
        setIsLoading(false);
        return;
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-8 md:py-12 mb-32 space-y-8">
        ...
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-2 mr-16">سيتم توليد كود المركبة تلقائياً حسب ترتيب الإدخال.</p>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 p-4 md:py-5 md:px-12 flex justify-between items-center z-50">
        ...
        <button type="submit" form="vehicle-form" disabled={isLoading}
          className="px-10 py-3.5 text-[13px] font-black text-white bg-slate-900 rounded-[1.25rem] hover:bg-indigo-600 disabled:opacity-50 transition-all shadow-lg flex items-center gap-2">
          {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Save size={18} />}
          {isLoading ? 'جاري الحفظ...' : 'حفظ المركبة'}
        </button>
      </div>
    </DashboardLayout>
  );
